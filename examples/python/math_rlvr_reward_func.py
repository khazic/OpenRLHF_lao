...
def reward_func(queries, prompts, labels, **kwargs):
    ...
    if print_limit > 0 and zero_score_details:
        print(f"[math_rlvr_reward_func] zero-score samples (showing up to {print_limit}):")
        for rank, detail in enumerate(zero_score_details[:print_limit], start=1):
            print(f"--- Zero-score #{rank} | sample_index={detail['index']} | score={detail['score']:.4f}")
            print(f"  Ground Truth : {detail['ground_truth']}")
            print(f"  Boxed Answer : {detail['boxed_answer']}")
